...
test("it should return none if no value exists at the specified path", function () {
  expect(equals(getIn({ x: { y: [1, 2, 3] } }, ['x', 'y', 6]), Maybe.None())).toBe(true);
});

test("it should return the value unwrapped in a some if a default value is provided", function () {
  expect(getIn({ x: { y: [1, 2, 3] } }, ['x', 'y', 0], 10)).toBe(1);
  expect(getIn({ x: { y: [1, 2, 3] } }, ['x', 'y', 2], 10)).toBe(3);
});

test("it should return the default value if no value exists at the specified path", function () {
