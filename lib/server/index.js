...
export default (function (App, _ref) {
  ...
  var render = function render(_ref2) {
    ...
    return Task.toPromise(andThen(traverse(app, Some(context)), function (_ref3) {
      ...
      return Task.of(template.replace(tags.lang, lang).replace(tags.dir, dir).replace(tags.state, "<script type='text/javascript'>window.APPLICATION_STATE = '" + EJSON.stringify(state) + "'</script>").replace(tags.view, html));
    }));
  };
