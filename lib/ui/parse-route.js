...
import { Maybe, Result, caseOf, foldl } from "../..";
...
export default (function (routes, location) {
  return foldl(routes, Result.Err({ path: "404", params: {}, uri: "404", query: {}, search: "" }), function (acc, _ref) {
    var _ref2 = _slicedToArray(_ref, 2),
        path = _ref2[0],
        route = _ref2[1];

    return caseOf(acc, { Ok: Result.Ok,
      Err: function Err(err) {
        return caseOf(matchURI(path, location.path), { Some: function Some(params) {
            return Result.Ok({ path: path, params: params, uri: location.path, query: location.query, hash: location.hash, search: location.search });
          },
          None: function None() {
